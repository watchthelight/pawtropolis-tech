...
const row = src.prepare(`SELECT sql FROM sqlite_master WHERE type='table' AND name='general_messages_embed'`).get();
if (!row?.sql) {
  console.error('table general_messages_embed not found in', SRC);
  process.exit(1);
}
dst.exec(row.sql);
