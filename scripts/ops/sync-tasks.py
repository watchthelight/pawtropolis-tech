...
BACKLOG_FILE = REPO_ROOT / "_BACKLOG.md"
...
PRIORITY_ORDER = ["Critical", "High", "Medium", "Low", "Nominal"]


def _backlog_priority(parsed: dict) -> str:
    pri = PRIORITY_MAP.get(parsed.get("priority") or "") or "Nominal"
    return pri


def _backlog_status_bucket(parsed: dict) -> str:
    status = (parsed.get("status") or "").upper()
    if status.startswith("WIP"):
        return "wip"
    if status.startswith("BLOCKED") or status.startswith("PAUSED"):
        return "blocked"
    return "active"


def _backlog_line(parsed: dict, include_status: bool = False) -> str:
    num = parsed["num"]
    title = clean_title(parsed["title"])
    pri = _backlog_priority(parsed)
    src = parsed["source_file"]
    line = f"- [ ] [{title}]({src}) `{pri}`"
    if include_status:
        raw = (parsed.get("status") or "").strip()
        if raw:
            line += f" `{raw}`"
    return line


def regenerate_backlog(open_parsed: list[dict]) -> None:
    """Rewrite _BACKLOG.md from open todo files. Grouped by status then priority."""
    wip: list[dict] = []
    blocked: list[dict] = []
    rest_by_pri: dict[str, list[dict]] = {p: [] for p in PRIORITY_ORDER}

    for p in open_parsed:
        bucket = _backlog_status_bucket(p)
        if bucket == "wip":
            wip.append(p)
        elif bucket == "blocked":
            blocked.append(p)
        else:
            rest_by_pri[_backlog_priority(p)].append(p)

    def _sort(xs: list[dict]) -> list[dict]:
        return sorted(xs, key=lambda x: x["num"])

    lines: list[str] = []
    lines.append("# Pawtropolis: Backlog")
    lines.append("")
    lines.append(
        "One-line index. Detail in [`todo/`](./todo/). Completed items move to "
        "[`done/`](./done/) and roll up into [`_DONE.md`](./_DONE.md)."
    )
    lines.append("")
    lines.append(
        "See [`docs/issue-system.md`](./docs/issue-system.md) for the file format, "
        "label vocabulary, and skill workflow."
    )
    lines.append("")
    lines.append(
        f"> Auto-generated by `scripts/ops/sync-tasks.py` at {now_utc_iso()}. "
        "Do not hand-edit; edit `todo/NNNNN.md` files instead."
    )
    lines.append("")
    ...
    lines.append("")
    if blocked:
        for p in _sort(blocked):
            lines.append(_backlog_line(p, include_status=True))
    else:
        lines.append("_(none)_")
    lines.append("")
    ...
    for pri in PRIORITY_ORDER:
        bucket_items = _sort(rest_by_pri[pri])
        if not bucket_items:
            continue
        any_rest = True
        lines.append(f"### {pri}")
...
def main():
    ...
    if TODO_DIR.exists():
        for path in sorted(TODO_DIR.glob("*.md")):
            ...
            if parsed:
                ...
                if not parsed.get("is_done"):
                    open_parsed.append(parsed)
    ...
    if not args.dry_run:
        regenerate_backlog(open_parsed)
        print(f"Backlog regenerated: {BACKLOG_FILE.relative_to(REPO_ROOT).as_posix()}")
        code, sha, _ = run_git(["rev-parse", "--short", "HEAD"])
        write_cursor(sha if code == 0 else "unknown")
        print(f"Cursor updated: {CURSOR_FILE.relative_to(REPO_ROOT).as_posix()}")
