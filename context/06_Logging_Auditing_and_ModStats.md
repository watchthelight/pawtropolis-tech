...
All moderator actions are recorded in the `action_log` table and posted to the guild's logging channel as "pretty cards" (rich embeds), so there's a persistent record of who did what and when.
...
1. `logging_channel_id` missing from configs table, so the query fails and returns null.
...
- Average response time (claim to decision)
