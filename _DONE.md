...
- [x] [Artist-rotation next-artist confirm TOCTOU race](done/00075.md) `High` New claimNextArtist() selects + rotates the next artist atomically in one transaction at confirm time, replacing the stale command-time artistId baked into the button. With the existing consumed_confirmations + ticket-spend guards, double/concurrent confirms can no longer double-assign. 5 tests.
- [x] [Patreon quantity>1 art tickets permanently under-granted](done/00077.md) `High` Added quantity_redeemed to patreon_art_granted (migration 081); /redeemreward confirm records the spend and the sweep re-grants the binary ticket role while granted - redeemed > 0. Closes the second-emoji-ticket loss. 7 tests. (Dup of #00085.)
- [x] [Patreon quantity>1 collapses onto a single binary ticket role](done/00085.md) `High` Fixed together with #00077: a redemption ledger (quantity_redeemed) lets the periodic sweep re-issue the ticket role until all granted tickets are spent.
- [x] [Cron-scheduled prod script imports undeclared @xenova/transformers](done/00089.md) `High` Removed the dead embed pipeline (embed-incremental/all/gold.mjs) and its 30-min prod cron entry; the package was deleted for a protobufjs RCE in #00003 and nothing in src/ or web/ consumes the embeddings. local-pipeline.mjs updated to drop the embed step.
- [x] [Re-skin /observatory as a classic Windows 95/98 desktop](done/00060.md) `Medium` Teal desktop, silver 3D-beveled window with navy title bar, sunken white chart wells, VGA palette, Tahoma, square. Token remap + page chrome only; 11 chart components untouched. Zero-JS held. Pushed + deployed --web; live and verified by screenshot.
