...
function getExcludedChannelIds(guildId: string): string[] {
  ...
  const row = db()
    .prepare("SELECT pulse_excluded_category_ids_json FROM guild_config WHERE guild_id = ?")
...
function weekStats(
  ...
  const communicators = (
    d
      ...
      .get(guildId, from, to, ...excludedChannelIds) as { count: number }
  ).count;
...
export function getNewsletterStats(guildId: string, range?: ResolvedRange): NewsletterStats {
  ...
  const voiceRows = (
    db().prepare("SELECT COUNT(*) as count FROM voice_session WHERE guild_id = ?").get(guildId) as {
      count: number;
    }
  ).count;
...
function engagementBucket(guildId: string, fromS: number, toS: number, excluded: string[]) {
  const d = db();
  const excludeClause =
    excluded.length > 0 ? `AND channel_id NOT IN (${excluded.map(() => "?").join(",")})` : "";
  ...
  const messages = (
    d
      ...
      .get(guildId, fromS, toS, ...excluded) as { count: number }
  ).count;
  ...
  const newMembers = (
    d
      ...
      .get(guildId, fromS, toS) as { count: number }
  ).count;
...
function detectJoinVelocity(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  return {
    ...
    trend: spike ? "up" : "down",
    suggestion: spike
      ? "Check if a raid or external promotion is driving the spike."
      : "Consider outreach or community events to boost visibility.",
  };
}
...
function detectRetentionCliff(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  if (cur < 20 || (prev > 0 && prev - cur > 10)) {
    return {
      ...
      trend: "down",
      suggestion:
        "Review the onboarding flow — are new members finding channels and conversations easily?",
    };
  }
  return null;
}
...
function detectLeaveSpike(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  return {
    ...
    trend: "up",
    suggestion:
      "Investigate recent events — server changes, conflicts, or moderation actions that may be driving departures.",
  };
}
...
function detectGhostRatio(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  return {
    ...
    trend: "up",
    suggestion:
      "Consider a welcome ping, starter prompts, or a guided intro channel to break the ice.",
  };
}
...
function detectChannelConcentration(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  const top = d
    ...
    .get(guildId, weekStart, guildId, weekStart) as
    | { channel_id: string; channel_name: string | null; cnt: number; pct: number }
    | undefined;
  ...
  return {
    ...
    trend: "flat",
    suggestion:
      "Consider whether this is healthy engagement or spam. Diversifying conversation across channels improves resilience.",
  };
}
...
function detectCommunicatorRatioDecline(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  return {
    ...
    trend: "down",
    suggestion:
      "Server activity is becoming more passive. Consider engagement events, topic prompts, or reducing lurker friction.",
  };
}
...
function detectVoiceTrend(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  const thisWeek = (
    d.prepare(voiceQuery).get(nowS, guildId, weekStart, nowS) as { minutes: number }
  ).minutes;
  const prevWeek = (
    d.prepare(voiceQuery).get(weekStart, guildId, prevStart, weekStart) as { minutes: number }
  ).minutes;
  ...
  if (
    thisWeek > 0 &&
    communicatorsThisWeek > 0 &&
    voiceUsersThisWeek < communicatorsThisWeek * 0.05
  ) {
    return {
      id: "voice-shallow",
      category: "engagement",
      ...
      trend: "flat",
      suggestion:
        "A small group is camping voice channels. Consider voice events or activities to broaden participation.",
    };
  }
  ...
  if (prevWeek > 0) {
    ...
    if (pctChange < -30) {
      return {
        ...
        trend: "down",
        suggestion:
          "Consider hosting a voice event, game night, or movie night to re-engage the community.",
      };
    }
...
function detectModmailResponseTime(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  const thisWeek = d.prepare(avgQuery).get(guildId, weekStart, nowS) as {
    avg_delay: number | null;
  };
  const prevWeek = d.prepare(avgQuery).get(guildId, prevStart, weekStart) as {
    avg_delay: number | null;
  };
  ...
  return {
    ...
    trend: "up",
    suggestion:
      "Assign modmail duty shifts or set up notifications so tickets get faster first responses.",
  };
}
...
function detectBehavioralFlagCluster(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  return {
    ...
    trend: "up",
    suggestion:
      "Review the flagged accounts for shared patterns (similar names, join timing, avatar style).",
  };
}
...
function detectGrowthSource(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  const top = d
    ...
    .get(totalJoins, guildId, weekStart) as
    | { invite_code: string; inviter_id: string | null; cnt: number; pct: number }
    | undefined;
...
function detectBoostHealth(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  const current = d
    .prepare("SELECT boost_count, boost_tier FROM guild_snapshot WHERE guild_id = ?")
    .get(guildId) as { boost_count: number; boost_tier: number } | undefined;
  if (!current) return null;

  const prev = d
    .prepare(
      "SELECT boost_count, boost_tier FROM guild_snapshot_log WHERE guild_id = ? AND date <= ? ORDER BY date DESC LIMIT 1"
    )
...
function detectOnlineRatio(guildId: string, ctx: InsightCtx): Insight | null {
  ...
  const current = d
    .prepare("SELECT member_count, online_count FROM guild_snapshot WHERE guild_id = ?")
  ...
  if (pctDiff < -25) {
    return {
      ...
      trend: "down",
      suggestion:
        "Fewer members are coming online. This may indicate declining interest or a seasonal pattern.",
    };
  }
...
export async function getLevelRoleStats(
  userId: string,
  tier: string
): Promise<LevelRoleStats | null> {
