...
export function getBackfillStats(): BackfillStats {
	try {
		const row = db().prepare(`SELECT * FROM backfill_stats WHERE id = 1`).get() as RawStatsRow | undefined;
		...
		const effectiveState =
			isStale && declaredState === 'running' ? 'paused' : declaredState;
		return {
...
export function getArchiveCounts(): { messages: number; reactions: number; sizeBytes: number | null } {
	try {
		const m = db().prepare(`SELECT COUNT(*) c FROM messages_archive`).get() as { c: number };
		const r = db().prepare(`SELECT COUNT(*) c FROM message_reactions_archive`).get() as { c: number };
