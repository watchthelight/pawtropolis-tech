...
export function getSecuritySnapshots(guildId: string, limit: number = 20): SecuritySnapshotSummary[] {
...
export function getSecuritySnapshotFull(guildId: string, snapshotId: number): SecuritySnapshotFull | null {
	const row = db().prepare(`
		SELECT id, created_at, role_count, channel_count, issue_count,
			critical_count, high_count, medium_count, low_count, content_hash,
