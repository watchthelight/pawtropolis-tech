...
export function getHomeMetrics(
	...
	if (includeModTier) {
		...
		const oldestUnreadModmail = db().prepare(
			...
			LIMIT 1`
		).get(guildId) as { id: number; user_id: string; app_code: string | null; name: string } | undefined;

		const oldestUnclaimed = db().prepare(
		...
		if (nextActions.length === 0) {
			nextActions.push({ kind: 'idle', label: 'Inbox zero', subtitle: 'Nothing waiting on you.', href: '/dashboard', weight: 0 });
		}
		...
		const recentDecisions = db().prepare(
			...
			LIMIT 15`
		).all(userId) as Array<{ type: string; action: string; app_id: string; ticket_id: number | null; created_at: number; subject_name: string }>;

		// modmail_message.created_at is text — convert via strftime in SQL.
		const recentModmail = db().prepare(
			...
			LIMIT 15`
		).all(guildId) as Array<{ type: string; action: string; app_id: string | null; ticket_id: number; created_at: number; subject_name: string }>;
