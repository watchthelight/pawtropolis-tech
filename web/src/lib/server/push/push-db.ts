...
export function upsertSubscription(
	...
	`).run(
		userId, tier, endpoint, p256dh, auth,
...
export function getSubscriptionsForDomain(domain: string): PushSubscriptionRow[] {
	...
	return db().prepare(`SELECT * FROM push_subscriptions WHERE ${col} = 1`).all() as PushSubscriptionRow[];
}

export function updateLastUsed(endpoint: string): void {
	db().prepare('UPDATE push_subscriptions SET last_used_at = unixepoch() WHERE endpoint = ?').run(endpoint);
}
...
export function updatePreferences(endpoint: string, prefs: PushPreferences): void {
	...
	if (prefs.review !== undefined) { sets.push('pref_review = ?'); vals.push(prefs.review ? 1 : 0); }
	if (prefs.modmail !== undefined) { sets.push('pref_modmail = ?'); vals.push(prefs.modmail ? 1 : 0); }
	if (prefs.flag !== undefined) { sets.push('pref_flag = ?'); vals.push(prefs.flag ? 1 : 0); }
	if (prefs.audit !== undefined) { sets.push('pref_audit = ?'); vals.push(prefs.audit ? 1 : 0); }
	if (sets.length === 0) return;
	vals.push(endpoint);
	db().prepare(`UPDATE push_subscriptions SET ${sets.join(', ')} WHERE endpoint = ?`).run(...vals);
}
...
export function deleteStaleSubscriptions(): number {
	...
	const result = db().prepare(
		'DELETE FROM push_subscriptions WHERE last_used_at IS NOT NULL AND last_used_at < ?'
	).run(cutoff);
