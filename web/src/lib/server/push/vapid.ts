...
export function getVapidPublicKey(): string {
	if (!_publicKey) {
		...
		if (!_publicKey) throw new Error('Missing VAPID_PUBLIC_KEY — run: npx web-push generate-vapid-keys');
	}
	return _publicKey;
}
...
export function getVapidPrivateKey(): string {
	if (!_privateKey) {
		...
		if (!_privateKey) throw new Error('Missing VAPID_PRIVATE_KEY — run: npx web-push generate-vapid-keys');
	}
	return _privateKey;
}
