...
<div class="splash">
	...
	<div class="hero-content">
		<!-- Neumorphic card behind content -->
		<div class="hero-card">
			<img
				src="/paw-logo.png"
				alt="Pawtropolis"
...
<style>
	...
	/* Neumorphic card container — soft raised surface */
	.hero-card {
		display: flex;
		flex-direction: column;
		align-items: center;
		...
		box-shadow:
			8px 8px 24px oklch(8% 0.01 var(--hue) / 0.5),
			-4px -4px 16px oklch(25% 0.02 var(--hue) / 0.15),
			inset 0 1px 0 oklch(30% 0.02 var(--hue) / 0.3);
	}
	...
	/* Discord sign-in button — neumorphic gradient */
	.discord-btn {
		position: relative;
		display: inline-flex;
		align-items: center;
		...
		border: 1px solid oklch(75% 0.15 330 / 0.3);
		box-shadow:
			0 4px 16px oklch(65% 0.2 330 / 0.3),
			0 1px 3px oklch(65% 0.2 330 / 0.2),
			inset 0 1px 0 oklch(90% 0.05 330 / 0.2);
	}

	.discord-btn:hover {
		transform: translateY(-2px);
		box-shadow:
			0 6px 24px oklch(65% 0.2 330 / 0.4),
			0 2px 8px oklch(65% 0.2 330 / 0.25),
			0 0 20px oklch(65% 0.2 330 / 0.15),
			inset 0 1px 0 oklch(90% 0.05 330 / 0.25);
	}

	.discord-btn:active {
		transform: translateY(1px) scale(0.98);
		box-shadow:
			0 2px 8px oklch(65% 0.2 330 / 0.2),
			inset 0 2px 4px oklch(30% 0.1 330 / 0.3);
	}

	/* Shimmer sweep — plays once, delayed slightly */
	.shimmer::after {
		content: '';
		position: absolute;
		inset: 0;
		...
		animation: shimmer-sweep 1.2s ease-in-out 1s forwards;
	...
	/* Pixel-art inspired detail — subtle dotted border accent on card */
	.hero-card::before {
		content: '';
		position: absolute;
		inset: 6px;
		border: 1px dashed oklch(35% 0.03 var(--hue) / 0.4);
