...
async function handleEnd(ctx: CommandContext<ChatInputCommandInteraction>): Promise<void> {
  ...
  const results = await withStep(ctx, "finalize_attendance", async () => {
    return finalizeGameAttendance(guild);
  });
...
async function handleAttendance(ctx: CommandContext<ChatInputCommandInteraction>): Promise<void> {
  ...
  const activeEvent = await withStep(ctx, "get_active", async () => {
    return getActiveGameEvent(guild.id);
  });
  ...
  if (!user) {
    ...
    if (activeEvent) {
      ...
      await withStep(ctx, "reply_live", async () => {
    ...
    await withStep(ctx, "reply_historical", async () => {
  ...
  const qualifiedCount = await withStep(ctx, "get_user_count", async () => {
    return getUserQualifiedGameCount(guild.id, user.id);
  });
...
async function handleAdd(ctx: CommandContext<ChatInputCommandInteraction>): Promise<void> {
  ...
  const success = await withStep(ctx, "add_attendance", async () => {
    return addManualGameAttendance(
      guild.id,
      user.id,
      minutes,
      interaction.user.id,
